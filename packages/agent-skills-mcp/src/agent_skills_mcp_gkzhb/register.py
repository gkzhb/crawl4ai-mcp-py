    ...
    for base_path in base_paths:
        try:
            ...
            for skill_path in glob.glob(
                ...
                skill = await parse_skill(skill_path, base_dir=cwd, cwd=cwd)
                if skill:
                    skills.append(skill)
                    skill_count += 1
        except Exception as e:
...
def register_tools(mcp: FastMCP, skill_list: List[Skill]) -> None:
    """Register all agent-skills tools to the MCP server."""
...
{skill.content}"""
    ...
    async def rescan_skills(ctx: Context) -> str:
        ...
        try:
            skills_list = await initialize_skills()
            await ctx.info(f"Successfully loaded {len(skills_list)} skills")

            result = f"Rescanned and loaded {len(skills_list)} skills\n\n"
